  ...
  if (existingSummaryComment) {
    ...
    if (tool_call_args) {
      ...
      if (existingSummaryComment.body !== newSummary) {
        ...
        await replyToReviewComment(existingSummaryComment.id, newSummary)
      }
    } else {
    ...
    if (tool_call_args) {
      await submitReview(
        ...
        uniqueComments, // Use filtered comments to avoid duplication
